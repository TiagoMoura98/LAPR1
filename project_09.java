...
public class project_09 {
        ...
        //In case we are not in interactive mode we proceed without user input
            ...
            for (int line = 0 ; line == dataLines ; line++){
                ...
                if (mArg == 1) {
                    ...
                    graphicModule((int)additionalInfo[0],(int) additionalInfo[1]);
                    GraficoPng(data[line], additionalInfo);
                }
                else {
                    //Compute values
                    double[][] resultRk4_2 = methodRk4(data[line], additionalInfo);
                    //Save to file
                    createFileCSV(resultRk4_2, names[line]+"_RK4.csv");
                    //Generate Graphics
                    //TODO Generate Graphics to File
                    graphicModule((int)additionalInfo[0],(int) additionalInfo[1]);
                    GraficoPng(data[line], additionalInfo);
    ...
    private static void menuOptions(double[] additionalInfo, double[] entryContent, String name) throws IOException {
        ...
        switch (optionSelection) {
            case 1:
                ...
                //Generate Graphics
                graphicModule((int)additionalInfo[0],(int) additionalInfo[1]);
                GraficoPng(entryContent, additionalInfo);
                //Go back to menu
    ...
    private static void graphicModule(int dias , int  populacao) throws FileNotFoundException {
        PrintWriter write = new PrintWriter("src\\graf.gp");
        ...
        write.println("plot 'graf.csv' using 1:2 with line title 'S'," + " 'graf.csv' using 1:3 with line title 'I', " + "'graf.csv' using 1:4 with line title 'R'");
        write.close();
    }
    ...
    private static void createFileCSV(double [][] resultado, String fileName) throws FileNotFoundException {
        PrintWriter write = new PrintWriter(fileName + ".csv");
        ...
        for (double[] doubles : resultado) {
            write.println(
                    String.format(Locale.US, "%6d", (int) doubles[0])
                            + ";" + String.format(Locale.US, "%.7f", doubles[1])
                            + ";" + String.format(Locale.US, "%.7f", doubles[2])
                            + ";" + String.format(Locale.US, "%.7f", doubles[3])
                            + ";" + String.format(Locale.US, "%.7f", doubles[4])
    ...
    private static String[] GraficoPng(double [] entryContent , double [] additionalInfo) throws IOException {
